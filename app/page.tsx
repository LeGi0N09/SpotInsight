  ...
  useEffect(() => {
    async function fetchData(showLoading = true) {
      ...
      const [statsRes, monthlyRes, profileRes] = await Promise.all([
        ...
        fetch(`${base}/api/spotify/monthly`, { cache: "no-store" }),
          ...
          {loading ? (
            ...
            <>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-6 animate-fadeIn">
                <KPI title="Total Plays" value={stats.totalPlays || 0} subtitle="Tracked in database" />
                <KPI title="Top Artists" value={stats.topArtists?.length || 0} subtitle="Unique artists" />
                <KPI title="Top Tracks" value={stats.topTracks?.length || 0} subtitle="Most played" />
                <KPI title="Genres" value={stats.topGenres?.length || 0} subtitle="Different genres" />
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-6 animate-fadeIn" style={{ animationDelay: '0.1s' }}>
                <div className="bg-[#111111] rounded-2xl p-4 h-80">
                  ...
                  <div className="space-y-3">
                    {monthly?.slice(-8).map((m: any, i: number) => {
                      const max = Math.max(...(monthly?.map((d: any) => d.plays) || [1]));
